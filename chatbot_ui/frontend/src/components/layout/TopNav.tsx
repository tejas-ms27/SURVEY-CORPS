...
// Chatbot is intentionally NOT a top-level tab — it now lives in the floating
// widget button (see FloatingChatButton). Plain labels, no numbering (round 3, item 10).
const TABS = [
  ...
  return (
    <header className="flex h-16 flex-none items-center gap-6 border-b border-white/10 bg-navy px-6 text-white">
      ...
      <nav className="flex items-center gap-1">
        {TABS.map((tab) => (
          <NavLink key={tab.key} to={`/${tab.key}`} className="relative">
            {({ isActive }) => (
              <div className="relative rounded-lg px-4 py-2 text-sm font-semibold tracking-wide text-white/60 transition-colors hover:text-white">
                {isActive && (
                  <motion.div
                    layoutId="topnav-active"
                    className="absolute inset-0 rounded-lg border border-teal/60 bg-white/10"
