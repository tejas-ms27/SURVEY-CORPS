...
def generate(
    ...
    Raises a clear error if GROQ_API_KEY is missing, rather than failing
    with a confusing requests/auth error deeper in the call stack.
    """
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "before running this script. Get a key from console.groq.com "
            "(NOT console.x.ai — different provider)."
        )
    ...
    if not response.ok:
        # Print the actual response body before raising — provider error
        # messages are specific and far more useful than the generic
        # "400 Client Error" text that raise_for_status() alone shows.
...
def handle_investigator_question(
    ...
    account_filter = resolve_account_filter(english_question, case["accounts"])
    ...
    id_result = try_id_lookup_answer(english_question, case)
    aggregation = None if id_result is not None else try_aggregation_answer(english_question, case)
    structured = (
        None if (id_result is not None or aggregation is not None)
        else try_structured_answer(english_question, case)
    )
    ...
    investigation = (
        ...
        else try_investigation_answer(english_question, case)
    )
    balance_trend = (
        None if (id_result is not None or aggregation is not None
                 or structured is not None or investigation is not None)
        else build_balance_trend_response(english_question, case)
    )
    timeseries_trend = (
        None if (id_result is not None or aggregation is not None or structured is not None
                 or investigation is not None or balance_trend is not None)
        else build_timeseries_response(english_question, case)
    )
    ...
    if id_result is not None:
    ...
    elif matches_graph_request(english_question):
        graph_result = build_graph_response(english_question, case, focus_account=account_filter)
        ...
        result = ask(collection, english_question, account_id=account_filter)
    ...
    structuring_alerts = collect_structuring_alerts(
        english_question, case, account_filter, structuring_by_account
    )
...
def run_cli():
    ...
    while True:
        ...
        # the cached structuring scan surfaced alongside.
        result = handle_investigator_question(
            question, case, collection, structuring_by_account=structuring_by_account
        )
