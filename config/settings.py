...
def warn_missing_extraction_keys() -> list[str]:
    """
    Soft, capability-aware key check used by the pipeline entry point.

    Requiring an OCR/vision key to parse a plain CSV/XLSX/TXT/digital-PDF is wrong: those
    formats need no LLM at all. So instead of hard-aborting the WHOLE run when a key is
    absent (the old require_extraction_keys behaviour), we LOG A CLEAR WARNING naming the
    degraded capability and let processing continue. Any file that genuinely needs a
    missing key then fails in isolation — recorded in files_failed with a specific reason
    by the per-file try/except — rather than taking down formats that never needed the key.

    This is the correct engineering decision independent of any test dataset: a keyless
    machine can still fully process structured statements, and a live demo never dies at
    startup over a capability it was never going to use.

    Returns the list of human-readable missing-capability messages (empty when all present).
    """
    missing = []
    if not GROQ1_KEY:
        missing.append(
            "GROQ1 missing — LLM schema-discovery / full-read fallback unavailable "
            "(deterministic parsing of structured statements is unaffected)."
        )
    if not GROQ2_KEY:
        missing.append(
            "GROQ2 missing — vision OCR for images and scanned PDFs unavailable "
            "(digital PDF / Excel / CSV / TXT are unaffected)."
        )
    for message in missing:
        logger.warning("settings.warn_missing_extraction_keys: %s", message)
    return missing
