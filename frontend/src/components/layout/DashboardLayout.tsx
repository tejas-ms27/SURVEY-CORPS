...
export function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-canvas">
      ...
      {/* Looping background video for the Open Case File workspace only. Sits behind
          the content (same layer as the side gutters), clears the top bar. */}
      {location.pathname === '/case' && (
        <>
          <video
            className="sc-case-bg-video"
            src="/case_background.mp4"
            autoPlay
            loop
            muted
            playsInline
            ...
            aria-hidden="true"
          />
          <div className="sc-case-bg-scrim" aria-hidden="true" />
        </>
      )}
